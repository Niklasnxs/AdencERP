...
try {
  ...
  if (!adminExists) {
    ...
    const insertEmployee = db.prepare(
      `INSERT INTO users (email, password, full_name, role, address, birthday, employment_type, email_access, email_login, email_password, mattermost_url, zoom_link, stundenliste_link)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    insertEmployee.run(
